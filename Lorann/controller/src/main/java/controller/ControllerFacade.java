...
public class ControllerFacade implements IController, IOrderStacker, IOrderPerformer {
	...
	private IUnit getNextTile(final IEntity entity) {
		return this.getModel().getLevel().getUnits()[entity.getX() + entity.getDirection().getX()][entity.getY()
				+ entity.getDirection().getY()];
	}
	...
	private void performInteraction(final IEntity entity, final IEntity target, final Interaction interaction) {
		switch (interaction) {
	...
	public void update() {
		...
		/*
		 * if (!this.getStackOrder().isEmpty()) { this.performOrder(); }
		 * this.updateEntities(); this.setStackOrder(new ArrayList<>());
		 */
	}
